...
export const ZH = {
  ...
  'bgtask.done': '完成',
  // The subagent report a completion notification carries. Hidden by default: the card is a
  // one-line summary and this text ran to 30122 characters in the sampled history.
  'bgtask.report': '查看报告',
  'bgtask.hideReport': '收起报告',
  ...
  // A slash command the owner ran in the terminal. The command text is verbatim and never
  // translated — it is what they typed.
  'status.command': '{cmd}',
  // The harness's own `[Request interrupted by user]` marker, shown as a notice rather than as a
  // bubble the owner never typed.
  'status.interrupted': '已中断本轮',
