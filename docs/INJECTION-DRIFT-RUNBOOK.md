...
- **Check the anchor is unique before you trust the first hit.** `findSource` returns the first
  source containing the needle, and which chunk that is comes down to readdir order — nothing a
...
| 2.1.263 | `child.sdkUrl`: a SECOND `("--sdk-url")` appeared — an argv helper (`l3t((s)=>s.startsWith("--sdk-url"),t)`, the is-this-print-mode check) — in a chunk that readdir lists BEFORE the allowlist getter's own. First-hit `findSource` walked back into a parameterised helper and returned `dHs=null` | shared-locator fix — `findSourceWhere` scans every occurrence and keeps the one whose surroundings are the getter (zero-arg `function NAME(){`, a `{status:` verdict within 300 chars) |
...
| 2.1.270 | `dispatch.policy`: first getter renamed `getBridgeDisabledReason` → `getBridgeDisabledDiagnosis` (now returns a diagnosis object; an `orgPolicyDenied` telemetry import appeared beside it). Only the export NAME the alias regex keys on moved (`alias-not-found partial={}`) | **widen** — E over both names: `getBridgeDisabled(?:Reason\|Diagnosis):([\w$]+),`; anchor/window/O/bpSubstr/rebind unchanged |
| 2.1.270 | `int.preflight`: the first check's early-return grew a block with an `orgPolicyDenied` telemetry call in front of it — `if(R)return{kind…}` → `if(R){if(R.orgPolicyDenied)db(…);return{kind…}}` (`rebind-re-not-found`) | **widen** — a lazy `[^]*?` between the first `if(…)` and its `return{kind`; second check, login ternary and trusted-device call unchanged |
| 2.1.271 | `child.sdkUrl`: the allowlist getter gained an early return before its argv read — `function J(){let n=K("--sdk-url")…` → `function J(){let e=x();if(e!==void 0)return{status:"ok",url:e};let n=K("--sdk-url")…`. The walk back to the enclosing name required a brace-FREE span (`function ([\w$]+)\(\)\{[^{}]*$`), so that object literal hid the getter and `dHs` went null (`could not locate`, `sdkUrlIdx:-1`) | shared-locator fix — the walk counts BRACE DEPTH: every `function NAME(){` in a 600-char lookback, keep the last one still open at the flag read (innermost enclosing zero-arg fn, any nesting). The `{status:` half of `accept` and the whole reject-site half are unchanged |
